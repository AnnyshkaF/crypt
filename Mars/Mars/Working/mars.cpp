...
void Mars::ReadFromFile(const char* filename, std::vector<u4byte>& v)
{
	...
	if (file.read(buffer.data(), size))
	{
		...
		t = 0;
		for (int p = 0; p < padding; p++)	//add left bytes
		{
			setByte(p, buffer[i++], t);
		}
		if (padding != 0)	//complete not-full block //last byte of data + 10s
		{
			setByte(padding - 1, 128, t);
			for (int i = padding - 2; i > -1; i--)	//0s after 1 after data
			{
				setByte(i, 0, t);
			}
			v.push_back(t);
...
void Mars::WriteToFile(const char* filename, const std::vector<u4byte>& v)
{
	...
	if (file.is_open())
	{
		...
		if (((((v[i + 1] >> 24) & 0xff) == 128)	//if full block was added 
		...
		{
			file << char(v[i] & 0xff);			//entire block before half-block
			file << char((v[i] >> 8) & 0xff);	//i=size()-2
			file << char((v[i] >> 16) & 0xff);
			file << char((v[i] >> 24) & 0xff);
			i++;
...
/*
void Mars::set4(u4byte value4, u4byte value3, u4byte value2, u4byte value1, u4byte& b)
{
	b |= value1 & 0xff;
	b |= (value2 & 0xff) << 8;
	b |= (value3 & 0xff) << 16;
