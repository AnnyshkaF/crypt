...
private:
	...
	void setByte(int pos, u4byte value, u4byte& b);

public:

	bool SetKey(const std::vector<u4byte>& k);
	void ReadFromFile(const char* filename, std::vector<u4byte>& v);
	void WriteToFile(const char* filename, const std::vector<u4byte>& v);
	void EncryptMessage(const std::vector<u4byte>& v, std::vector<u4byte>& res);
	void DecryptMessage(const std::vector<u4byte>& v, std::vector<u4byte>& res);
};